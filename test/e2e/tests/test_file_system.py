...
- Verify status fields populated (fileSystemID, status, ackResourceMetadata.arn)
- Policy CRUD lifecycle (add, verify, remove, verify)
- Policy included at create time
...
@pytest.fixture(scope="module")
def file_system_with_policy(s3files_client):
    """Create a FileSystem with a policy in the initial spec."""
    resource_name = random_suffix_name("ack-s3files-pol", 24)

    replacements = _get_replacements()
    replacements["FILE_SYSTEM_NAME"] = resource_name
    account_id = get_account_id()
    replacements["POLICY"] = _make_sample_policy(account_id)

    resource_data = load_s3files_resource(
        "file_system_with_policy",
        additional_replacements=replacements,
    )
    logging.debug(resource_data)
...
class TestFileSystem:
    ...
    def test_create_and_wait_for_available(self, s3files_client, simple_file_system):
        """Test that creating a FileSystem CR reaches available and has all status fields."""
    ...
    def test_policy_lifecycle(self, s3files_client, simple_file_system):
        """Test the full policy CRUD lifecycle: add, verify, remove, verify."""
        (ref, cr) = simple_file_system
        ...
        assert k8s.wait_on_condition(
            ...
            wait_periods=AVAILABLE_WAIT_PERIODS,
            period_length=AVAILABLE_WAIT_PERIOD_LENGTH,
        ), "FileSystem did not reach Synced=True after policy add"
        condition.assert_synced(ref)
    ...
    def test_policy_at_create_time(self, s3files_client, file_system_with_policy):
        """Test that a policy included in the initial CR spec is synced after create."""
        (ref, cr) = file_system_with_policy
        ...
        # Verify the policy was applied in AWS after the FS reached available
        aws_policy = _get_aws_file_system_policy(s3files_client, file_system_id)
        assert aws_policy is not None, \
            "Policy not found — controller did not sync policy from initial spec after create"

        # Verify the policy is reflected in the CR spec
        cr = k8s.get_resource(ref)
        cr_policy = cr.get("spec", {}).get("policy")
        assert cr_policy is not None, "Policy not synced back to CR spec"
    ...
    def test_add_sync_config(self, s3files_client, simple_file_system):
        ...
        assert len(import_rules) == 1, \
            f"Expected exactly 1 import data rule, got {len(import_rules)}"

        expiration_rules = aws_sync.get("expirationDataRules", [])
        assert len(expiration_rules) == 1, \
            f"Expected exactly 1 expiration data rule, got {len(expiration_rules)}"
